...
library SparseMerkleTree {
    ...
    function setMaxDepth(UintSMT storage self, uint128 maxDepth_) internal {
        ...
        require(maxDepth_ <= MAX_DEPTH_HARD_CAP, "Max depth is greater than hard cap");
    ...
    function root(UintSMT storage self) internal view onlyInitialized(self) returns (uint256) {
        return self.merkleRoot;
    }

    function node(UintSMT storage self, uint256 nodeHash_) internal view returns (Node memory) {
        return self.nodes[nodeHash_];
    }

    function proof(UintSMT storage self, uint256 index_) internal view returns (Proof memory) {
    ...
    function _add(
        ...
        if (node_.nodeType == NodeType.EMPTY) {
            ...
            if ((newLeaf_.index >> depth_) & 1 == 1) {
                nextNodeHash_ = _add(self, newLeaf_, node_.childRight, depth_ + 1);
                ...
                nextNodeHash_ = _add(self, newLeaf_, node_.childLeft, depth_ + 1);
    ...
    function _pushLeaf(
        ...
        if (newLeafBitAtDepth_ == oldLeafBitAtDepth_) {
            uint256 nextNodeHash_ = _pushLeaf(self, newLeaf_, oldLeaf_, depth_ + 1);
    ...
    function _addNode(UintSMT storage self, Node memory node_) private returns (uint256) {
    ...
    function _getNodeHash(Node memory node_) private pure returns (uint256) {
        ...
        if (node_.nodeType == NodeType.LEAF) {
            nodeHash_ = PoseidonFacade.poseidon3([node_.index, node_.value, uint256(1)]);
        } else if (node_.nodeType == NodeType.MIDDLE) {
            nodeHash_ = PoseidonFacade.poseidon2([node_.childLeft, node_.childRight]);
        }
